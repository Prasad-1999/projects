...
class book(reservation, price_changes, seats_changes, refund_percent_changes):
    ...
    def availablity(self):
        ...
        if self.seats[self.compartment] > 0:
            ...
            self.coupon = 'C' + \
                self.fname[0] + self.lname[0] + \
                self.gender[0] + self.compartment
    ...
    def database(self):
        with open('bookings.csv', 'a', newline='') as file:
            ...
            writer.writerow([self.coupon, self.fname, self.lname, self.age, self.gender, self.compartment,
                            self.availability, self.price[self.compartment], self.mode, self.completion])
        file.close()
    ...
    def booked_ticket_printing(self):
        ...
        print("For confirmed ticket -", self.confirm_refund_percent,
              "% of ticket price refund.")
        print("For waiting ticket -", self.waiting_refund_percent,
              "% of ticket price refund.")
...
class cancel(reservation, price_changes, seats_changes, refund_percent_changes):
    ...
    def refund(self):
        if self.coupon[0] == 'C':
            ...
            self.re = self.price[self.coupon[4:]] - \
                self.price[self.coupon[4:]]*(self.confirm_refund_percent/100)
            ...
            self.re = self.price[self.coupon[4:]] - \
                self.price[self.coupon[4:]]*(self.waiting_refund_percent/100)
    ...
    def cancelled_ticket_printing(self):
        ...
        print("Your Coupon Code:", self.coupon)
        print("Your refund is", self.re, "rupees.")
        print("You saved (", self.price[self.coupon[4:]], "-", self.re,
              ") =", self.price[self.coupon[4:]]-self.re, "rupees.")
        print("Process completed.")
        print()
